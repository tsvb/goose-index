...
describe("SetlistMinimal (document)", () => {
  ...
  it("the row's web fallback is the exact release when a container is known, and search otherwise", () => {
    const withContainer = renderToStaticMarkup(
      <SetlistMinimal entries={[entry({ song: "Hot Tea" })]} showDate="2024-04-20" venue="The Salt Shed" containerId={46887} />,
    );
    ...
    const withoutContainer = renderToStaticMarkup(
      <SetlistMinimal entries={[entry({ song: "Hot Tea" })]} showDate="2024-04-20" venue="The Salt Shed" containerId={null} />,
    );
