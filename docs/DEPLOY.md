...
- **Preview deployments have no database — and fail, on purpose.** Every connection
  string on Vercel is scoped to Production only, and the build prerenders
  `/sitemap.xml`, which needs one — so the Vercel check on a PR is red on every
  preview. Accepted trade-off (2026-08-30): CI is the check that gates a merge, and
  production deploys are unaffected. To make previews build instead, tick Preview on
  `DATABASE_URL` (or the integration's `POSTGRES_URL`) — previews would then read the
  production database, which is safe: reads only, and the migrate gate (below) means
  a preview can never touch the schema.
