...
export async function allShowDates(): Promise<string[]> {
  return cachedQuery("allShowDates", [], async () => {
  const rows = await db
...
export async function allSongSlugs(): Promise<string[]> {
  return cachedQuery("allSongSlugs", [], async () => {
  const rows = await db
...
export async function allYears(): Promise<number[]> {
  return cachedQuery("allYears", [], async () => {
...
export async function allVenueIds(): Promise<number[]> {
  return cachedQuery("allVenueIds", [], async () => {
  const rows = await db
...
export async function allTourIds(): Promise<number[]> {
  return cachedQuery("allTourIds", [], async () => {
  const rows = await db
