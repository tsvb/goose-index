...
export async function searchVenues(q: string, limit = 12): Promise<VenueRow[]> {
  const like = `%${q}%`;
  return db
    .select({
      venueId: venues.venueId,
    ...
    .from(venues)
    .leftJoin(shows, eq(shows.venueId, venues.venueId))
    .where(sql`(${venues.name} ilike ${like} or ${venues.city} ilike ${like})`)
    .groupBy(venues.venueId)
    .having(sql`count(${shows.showId}) > 0`)
...
export async function searchTours(q: string, limit = 8): Promise<TourRow[]> {
  const like = `%${q}%`;
  return db
    .select({
      tourId: tours.tourId,
    ...
    .from(tours)
    .leftJoin(shows, eq(shows.tourId, tours.tourId))
    .where(sql`${tours.name} ilike ${like}`)
    .groupBy(tours.tourId)
    .having(sql`count(${shows.showId}) > 0`)
    .orderBy(sql`min(${shows.showDate}) desc nulls last`)
    .limit(limit);
